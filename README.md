...
Left 4 Dead 2의 매치메이커가 원치 않는 서버에 연결할 때 자동으로 메인 타이틀로 복귀시키는 프로그램입니다.
...
L4D2 매치메이커는 공식/사설 서버를 구별하지 못하고, 사설 서버 운영자측에 차단 (소위 '셀프밴')을 요청한들 그걸 들어주리라는 장담은 불가하며, 방화벽으로 막으면 클라이언트가 연결을 10회씩 재시도하므로 시간이 낭비됩니다. 
...
상기 문제를 해결하여 L4D2 클라이언트가 특정 사설 서버에 접속을 시도하는 즉시 메인 타이틀로 넘어가도록 만들었습니다.

**이 프로그램은 클라이언트 자체를 위변조하는 것이 아니라 Windows 네트워크 스택에서 특정 서버행 패킷만 가로채 응답을 대신 돌려주는 방식입니다.** [WinDivert](https://reqrypt.org/windivert.html) 커널 드라이버로 차단 대상 서버행 outbound UDP 패킷을 가로채고, Source 엔진 프로토콜을 모방한 응답을 주입하여 L4D2 클라이언트가 즉시 연결을 포기하도록 만듭니다. 자세한 내용은 아래 '동작 원리'를 참고하세요.
...
3. L4D2 게임을 켜서 평소처럼 플레이하세요. 차단한 서버에 접속을 시도하는 즉시 메인 타이틀로 복귀됩니다.
...
클라이언트는 `auth_protocol = 0`을 읽고 `PROTOCOL_STEAM(3)`이 아님을 확인한 즉시 `Disconnect()`를 호출합니다. 타임아웃이나 재시도 없이 **즉시** 연결이 해제되며, 메시지 없이 메인 타이틀 UI로 복귀합니다.
